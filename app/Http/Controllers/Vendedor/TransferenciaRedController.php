...
class TransferenciaRedController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware(function ($request, $next) {
            $user = Auth::user();
            if (!$user->esVendedor() && !$user->esLider()) {
                abort(403, 'Acceso denegado. Solo vendedores y líderes.');
            }
            return $next($request);
        });
    }
    ...
    public function index()
    {
        ...
        return view('vendedor.transferencia_red_index', compact('solicitudes'));
    }
    ...
    public function show($id)
    {
        ...
        return view('vendedor.transferencia_red_show', compact('solicitud', 'nodos'));
    }
    ...
    public function aprobar(Request $request, $id)
    {
        ...
        try {
            ...
            return redirect()->route('vendedor.transferencia-red.index')
    ...
    private function transferirRed($nodosIds, $nuevoReferidorId)
    {
        ...
        // Actualizar contador de referidos del nuevo vendedor
        $vendedor = User::find($nuevoReferidorId);
        if ($vendedor) {
            $vendedor->total_referidos = User::where('referido_por', $nuevoReferidorId)->count();
            $vendedor->save();
        }
    }
}
